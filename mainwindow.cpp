...
//    auto cursor_map = this->ui->minimap->textCursor(); cursor_map.mergeBlockFormat(blockFmt); this->ui->minimap->setTextCursor(cursor_map);

    this->ui->textEdit->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);

    MyMinimapWidget* p_minimap = this->ui->minimap;
    this->ui->textEdit->connectMinimap(p_minimap->p_textEdit);
}
